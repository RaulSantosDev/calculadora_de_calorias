...
export default function ActivityList({activities, dispatch} : ActivityListProps) {
    ...
    return (
        <>
                ...
                { isEmptyActivities ? <p className="my-8 text-2xl text-black-300 text-center">La lista está vacía</p> :                
                    activities.map((activity) => (
                        <div 
                            key={activity.id}
                            className="px-5 py-10 bg-white mt-5 flex justify-between rounded-sm shadow"    
                        >
